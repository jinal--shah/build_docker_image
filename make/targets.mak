...
BLGT:=$(strip $(BUILD_LIBS_GIT_TAG))
BLDIR:=$(BUILD_LIBS_LIB_DIR)
...
check_includes: ## check we use the desired build_ami version
	@if [[ -z "$(BLGT)" ]]; then                                                   \
	    ...
	    echo -e "\033[1;37mChecking $(BLDIR) version: $(BLGT)\033[0m";             \
	    [[ -d $(BLDIR) ]]                                                          \
	    && cd $(BLDIR)                                                             \
	    && [[ $$(git describe --tags --match [0-9]*.[0-9]*.[0-9]*) == "$(BLGT)" ]] \
	    && echo "... using version: $(BLGT)";                                      \
	fi
...
check_for_changes: ## check project_dir and build_ami for uncommitted changes.
	...
	@echo -e "\033[1;37mChecking for uncommitted changes in $(BLDIR)\033[0m"
	@cd $(BLDIR)                                                        \
	&& if git diff-index --quiet HEAD -- ;                              \
	then                                                                \
	    echo "... none found.";                                         \
	else                                                                \
	    echo -e "\033[0;31m[ERROR] local changes in $(BLDIR)\033[0m";   \
	    echo "... Commit them (tag the commit if wanted), then build."; \
	    exit 1;                                                         \
	fi;
...
.PHONY: run_docker_build
run_packer: ## invoke Packer build
	 docker build $(PACKER_DEBUG) "$(PACKER_JSON)"
